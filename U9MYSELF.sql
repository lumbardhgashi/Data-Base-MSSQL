...
--Shkruani një query ku do të listoni të gjitha automobilet qe i takojne prodhuesit që ka id e
--barabartë me 13. (Të përdoren single row subqueries)

Select a.modeli ,k.prodh_ID
From Automobili a join Katalogu k
 on a.automobil_ID = k.a_ID 
 Where k.prodh_ID in (Select p.prodhues_ID
                        From Prodhuesi p
						where  p.prodhues_ID = '13');

/*
2. Shkruani një query ku do të listoni te gjithe prodhuesit e automobilit te klases 'SUV'. (Të
përdoren single row subqueries)
*/
Select Distinct p.emri_P
From Prodhuesi p join Katalogu k 
on p.prodhues_ID = k.prodh_ID
join Automobili a 
on a.automobil_ID = k.a_ID
Where a.klasa in (Select a.klasa
                  From Automobili a
				  Where a.klasa ='SUV');

/*
3. Shkruani nje query ku do te listoni te gjithe modelet e automobilit te cilet jane prodhuar
nga BMW. (Të përdoren single row subqueries)
*/
Select Distinct a.modeli
From Automobili a Left Join Katalogu k
on a.automobil_ID = k.a_ID
Left join Prodhuesi p
on p.prodhues_ID = k.prodh_ID
Where p.emri_P in (Select p.emri_P
                   From Prodhuesi p
				   Where p.emri_P = 'BMW');
				  
/*
4. Shkruani nje query qe do te listoni te gjithe prodhuesit e automobileve qe ne katalog e
kanë çmimin e shitjes me te ulet se mesatarja e çmimit të të gjithë automobilave te klases
'SUV'. (Të përdoren single row subqueries)
*/
...
join Prodhuesi p
on p.prodhues_ID = k.prodh_ID
Where a.klasa='SUV' AND  k.qmimi_shitjes < (Select avg(k.qmimi_shitjes) 'Cmimi Mesatar' 
                                             From Automobili a JOIN Katalogu k 
											 ON a.automobil_ID = k.a_ID JOIN
											 Prodhuesi p
											 on p.prodhues_ID = k.prodh_ID
						                     Where   a.klasa = 'SUV')
order by k.qmimi_shitjes desc;
										 

/*
5. Shkruani një query ku do të numëroni të gjithë automobilat me cmimin e blerjes më të
lartë se mesatarja e qmimit te blerjes se automobilit te klases 'coupe'. (Të përdoren single
row subqueries)
*/
Select a.qmimi_blerjes , COUNT(a.automobil_ID)[Automobili ID ]
From Automobili a
where a.qmimi_blerjes > (Select AVG(a.qmimi_blerjes) [Cmimi Mesatar i Blerjes]
                         From Automobili a 
                         where a.klasa='coupe')
                         Group by a.qmimi_blerjes;
